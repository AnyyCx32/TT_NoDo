...
// ============================================================================
static const char FW_VERSION[] = "TT-NODO-R11-TEST-MODEM";
...
// GP29 lleva VSYS dividido entre 3 en la Pico. Permite que el nodo vigile su
// propia alimentacion y deje constancia de las caidas en la bitacora.
#define VSYS_ADC_PIN    A3
...
// Periodo de transmision. ThingSpeak descarta toda actualizacion que llegue
// antes de 15 s de la anterior, asi que el periodo no lo fija el nodo sino el
// servicio. El espaciado real no es SEND_PERIOD_MS: el temporizador se ancla en
// el instante del HTTP OK anterior, de modo que al periodo se le suma la
// duracion del ciclo nuevo. Con 15000 se midieron 16.08 s de mediana el 20 de
// septiembre de 2026; con 14800 la mediana queda en unos 15.8 s, que es lo mas
// cerca del limite que se puede operar con margen.
static const uint32_t SEND_PERIOD_MS = 14800;
// Piso duro medido desde el ultimo envio ACEPTADO, no desde el arranque del
// ciclo. La duracion del ciclo varia entre 0.85 y 23 s, asi que sin este tope
// una racha de ciclos rapidos podria colocar dos entregas a menos de 15 s y
// ThingSpeak descartaria la segunda en silencio. Con el, el rechazo por
// frecuencia es imposible por construccion.
static const uint32_t TS_MIN_GAP_MS = 15300;
...
bool imuCalibrationFloorLoaded = false;
...
// Intentos de montaje al arrancar, con pausa entre ellos. La tarjeta puede
// necesitar unos cientos de milisegundos si el riel sube despacio.
static const uint8_t  SD_MOUNT_ATTEMPTS  = 5;
static const uint32_t SD_MOUNT_RETRY_MS  = 300;
...
// ============================================================================
// La rama de 5 V que alimenta a la Pico llega a VSYS, y GP29 la entrega
// dividida entre 3. Se muestrea en cada vuelta del lazo y se conserva el
// minimo y el maximo de cada intervalo de registro, de modo que una caida
// breve (por ejemplo durante una rafaga de transmision del SIM7600, que pide
// cerca de 2 A) quede capturada aunque dure pocos milisegundos.
float vsysNow_V = 0.0f;
float vsysMin_V = 99.0f;
float vsysMax_V = 0.0f;
bool vsysAlertActive = false;

// Por debajo de este valor el regulador de 3V3 de la Pico empieza a quedarse
// sin margen y el SIM7600 puede reiniciarse en plena transmision.
static const float VSYS_ALERT_LOW_V   = 4.40f;
static const float VSYS_ALERT_CLEAR_V = 4.60f;   // histeresis para no repetir
// Piso de plausibilidad: por debajo de esto la lectura no puede ser real con el
// nodo en marcha, y se descarta como muestra invalida del ADC.
static const float VSYS_VALID_MIN_V   = 2.00f;
// Muestras consecutivas bajo el umbral antes de declarar la caida.
static const uint8_t VSYS_ALERT_SAMPLES = 5;
uint8_t vsysLowStreak = 0;
// Tiempo tras el arranque durante el cual no se evaluan alertas: el riel
// todavia esta subiendo y sus lecturas bajas son legitimas.
static const uint32_t VSYS_ALERT_ARM_MS = 3000;
// Minimo propio de la bitacora. El del CSV se reinicia con cada fila de
// 50 ms, de modo que la instantanea periodica lo leia recien reiniciado y
// reportaba siempre el valor centinela.
float vsysMinSnapshot_V = 99.0f;
...
enum LteState : uint8_t {
  ...
  LTE_DONE,
  LTE_FAIL,
  // Los estados de reanimacion se anaden AL FINAL a proposito. Los numeros de
  // los estados anteriores se publican como estadoPrevio en cada linea
  // HTTP FALLO, y reordenarlos volveria ilegibles las bitacoras ya levantadas.
  LTE_RECOVER_PULSE, LTE_RECOVER_SETTLE,
  LTE_RECOVER_PROBE, LTE_RECOVER_WAIT_PROBE
};

LteState lteState = LTE_IDLE;
uint32_t lteStateStartMs = 0;
// Instante en que arranco el ciclo de transmision y estado desde el que se
// entro a LTE_FAIL. Sin consola conectada, ese estado es lo unico que permite
// distinguir un fallo de red de uno del servidor al revisar la bitacora.
uint32_t lteCycleStartMs = 0;
// Reinicios espontaneos del modem observados durante la sesion.
uint32_t sesionReiniciosModem = 0;
uint32_t lastModemResetMs = 0;
// Margen tras el arranque del nodo: el primer RDY del modem es legitimo.
static const uint32_t MODEM_RESET_GRACE_MS = 30000;
// Ventana de espera para que el SIM7600 acepte comandos AT tras el pulso de
// PWRKEY. El fabricante indica del orden de 12 a 15 s desde que se suelta la
// senal; se concede margen porque quedarse corto tiene consecuencias graves,
// segun se explica en la nota de setup().
static const uint32_t MODEM_ESPERA_AT_MS = 25000;
// Reanimacion del modem. Un HTTPACTION que expira deja al SIM7600 sin contestar
// ni a un AT pelon: el 20 de septiembre de 2026 eso produjo dos huecos de 5.7 y
// 7.1 minutos, uno de los cuales solo termino cuando el nodo perdio energia.
static const uint8_t  MODEM_DEAF_STREAK_TRIGGER = 3;     // VARIANTE DE PRUEBA
static const uint32_t MODEM_RECOVERY_COOLDOWN_MS = 180000;
static const uint32_t PWRKEY_PULSE_MS = 900;
static const uint32_t PWRKEY_SETTLE_OFF_MS = 3000;
static const uint32_t PWRKEY_SETTLE_ON_MS = 5000;
static const uint32_t MODEM_RECOVERY_PROBE_MS = 3000;
LteState lteFailedFromState = LTE_IDLE;
uint32_t lastSendMs = 0;
uint32_t lastRetryMs = 0;
// Instante del ultimo envio que ThingSpeak acepto. Gobierna TS_MIN_GAP_MS y es
// distinto de lastSendMs, que tambien se mueve al arrancar un ciclo.
uint32_t lastAcceptedSendMs = 0;
// Esperas agotadas consecutivas en LTE_WAIT_AT: el sintoma de un modem mudo.
uint8_t  modemDeafStreak = 0;
uint32_t modemRecoveryStartMs = 0;
uint32_t lastModemRecoveryMs = 0;
uint32_t sesionRecuperacionesModem = 0;
// Pulsos aplicados en la reanimacion en curso. PWRKEY conmuta, asi que un solo
// pulso no basta: hay que pulsar, preguntar, y solo si sigue mudo volver a
// pulsar. Ver la nota de LTE_RECOVER_PULSE.
uint8_t  modemRecoveryPulses = 0;
static const uint8_t MODEM_RECOVERY_MAX_PULSES = 2;
...
// ============================================================================
// BITACORA DE EVENTOS EN LA microSD
// ============================================================================
// Todo lo que el programa informaba unicamente por USB se guarda tambien aqui,
// de modo que el nodo pueda operar desconectado de la computadora y aun asi
// dejar la evidencia que piden las fichas de prueba del Capitulo 4:
// valores de calibracion del arranque, estado de red, codigos HTTP con su
// instante, fallos de almacenamiento y un resumen periodico de la sesion.
//
// Cada evento abre, escribe y cierra el archivo. Los eventos son poco
// frecuentes, de modo que el costo es despreciable frente al registro de
// vibraciones a 20 Hz, y a cambio ningun evento se pierde si se corta la
// alimentacion sin apagar el nodo por software.
static const char EVENT_LOG_FILE[] = "EVENTOS.LOG";

uint32_t sesionEnviosOk = 0;
uint32_t sesionEnviosFallidos = 0;
uint32_t sesionFallosSd = 0;
uint32_t lastEventStatusMs = 0;
uint32_t lastEventSummaryMs = 0;

static const uint32_t EVENT_STATUS_MS  = 30000;   // instantanea de estado
static const uint32_t EVENT_SUMMARY_MS = 300000;  // resumen acumulado

void logEvent(const char *texto) {
  Serial.println(texto);
  if (!sdOK) return;

  File f = SD.open(EVENT_LOG_FILE, FILE_WRITE);
  if (!f) return;

  char fecha[11];
  char hora[9];
  formatDateTime(fecha, sizeof(fecha), hora, sizeof(hora));

  f.print(fecha); f.print(' '); f.print(hora);
  f.print(" ["); f.print(millis()); f.print("] ");
  f.println(texto);
  f.close();
}

void logEvent(const String &texto) {
  logEvent(texto.c_str());
}

// Instantanea compacta del estado del nodo. Es la evidencia de que el sistema
// siguio operando durante toda la sesion, incluso sin consola conectada.
void logStatusSnapshot() {
  String s = "ESTADO src=" + String((int)txSource) +
             " imu=" + String((int)imuHealth) +
             " gnss=" + String(gpsUsable ? 1 : 0) +
             " sats=" + String(gpsSats) +
             " hAcc=" + String(gpsHAcc_mm / 1000.0f, 2) +
             " pdop=" + String(gpsPDOP_centi / 100.0f, 2) +
             " lat=" + String(gpsLat, 6) +
             " lon=" + String(gpsLon, 6) +
             " vel=" + String(txSpeedKmh, 1) +
             " reposo=" + String(stillDetected ? 1 : 0) +
             " zupt=" + String(zuptApplied ? 1 : 0) +
             " vib=" + String(vibMagLin_g, 4) +
             " sd=" + String(sdOK ? 1 : 0) +
             " vsys=" + String(vsysNow_V, 2) +
             " vsysMin=" + String(vsysMinSnapshot_V, 2);
  logEvent(s);
  vsysMinSnapshot_V = 99.0f;   // se reinicia para el siguiente intervalo
}

void logSessionSummary() {
  String s = "RESUMEN t=" + String(millis() / 1000) + "s" +
             " enviosOk=" + String(sesionEnviosOk) +
             " enviosFallidos=" + String(sesionEnviosFallidos) +
             " fallosSD=" + String(sesionFallosSd) +
             " reiniciosModem=" + String(sesionReiniciosModem) +
             " archivoVib=" + String(vibFilename);
  logEvent(s);
}

// Nombre legible de la causa del ultimo reinicio. Es el dato que distingue una
// falla de alimentacion de un cuelgue del programa: si el nodo se reinicia solo
// y aqui aparece BROWNOUT o GLITCH, el problema es electrico, no de firmware.
const char *resetReasonName() {
  switch (rp2040.getResetReason()) {
    case RP2040::PWRON_RESET:    return "PWRON (encendido normal)";
    case RP2040::RUN_PIN_RESET:  return "RUN (pin de reset externo)";
    case RP2040::SOFT_RESET:     return "SOFT (reinicio por software)";
    case RP2040::WDT_RESET:      return "WDT (perro guardian: el programa se colgo)";
    case RP2040::DEBUG_RESET:    return "DEBUG (depurador)";
    case RP2040::GLITCH_RESET:   return "GLITCH (perturbacion en la alimentacion)";
    case RP2040::BROWNOUT_RESET: return "BROWNOUT (la tension cayo por debajo del minimo)";
    default:                     return "DESCONOCIDA";
  }
}

// Muestreo de la alimentacion. Se llama en cada vuelta del lazo.
void sampleVsys() {
  // Mediana de cinco conversiones en vez de una sola.
  //
  // El ADC del RP2040 entrega ocasionalmente una muestra muy por debajo del
  // valor real. Como el minimo del intervalo se queda con la lectura mas baja,
  // una sola de esas espurias bastaba para registrar caidas que nunca
  // ocurrieron: se observo vsysMin=2.38 V alimentando por USB, donde el riel
  // no puede bajar de los 5 V menos la caida del diodo. La mediana descarta
  // los valores aislados sin retrasar la deteccion de una caida real, que se
  // manifiesta en muestras consecutivas.
  int m[5];
  for (uint8_t i = 0; i < 5; ++i) m[i] = analogRead(VSYS_ADC_PIN);
  ...
  float lectura = m[2] * 3.3f / 4095.0f * 3.0f;

  // Descarte de lecturas imposibles. Con el nodo en marcha, VSYS no puede estar
  // realmente por debajo de VSYS_VALID_MIN: el convertidor de la Pico no
  // sostendria los 3V3 y el programa no estaria ejecutandose para medirlo. Las
  // primeras conversiones tras el arranque, antes de que el ADC se asiente,
  // producen valores de ese tipo y darian una alerta de bajo voltaje falsa
  // en cada encendido, justo en la evidencia que se quiere conservar limpia.
  if (lectura < VSYS_VALID_MIN_V) return;

  vsysNow_V = lectura;
  if (vsysNow_V < vsysMinSnapshot_V) vsysMinSnapshot_V = vsysNow_V;
  if (vsysNow_V < vsysMin_V) vsysMin_V = vsysNow_V;
  if (vsysNow_V > vsysMax_V) vsysMax_V = vsysNow_V;

  // Se exigen varias muestras consecutivas por debajo del umbral para no
  // confundir el ruido del ADC con una caida real. A la cadencia del lazo, son
  // unas pocas decenas de milisegundos: sigue siendo suficiente para atrapar
  // el hundimiento de una rafaga de transmision.
  // Durante los primeros segundos el riel esta subiendo de verdad, y una
  // lectura de 3.3 V mientras arranca no es una caida sino la rampa de
  // encendido. Anotarla como alerta ensuciaria la bitacora en cada arranque.
  if (millis() < VSYS_ALERT_ARM_MS) {
    vsysLowStreak = 0;
    return;
  }

  if (vsysNow_V < VSYS_ALERT_LOW_V) {
    if (vsysLowStreak < 255) vsysLowStreak++;
  } else {
    vsysLowStreak = 0;
  }

  // Se deja constancia inmediata de la caida. Como cada evento abre, escribe y
  // cierra el archivo, la anotacion sobrevive aunque el nodo se reinicie
  // enseguida, que es precisamente lo que se quiere diagnosticar.
  if (!vsysAlertActive && vsysLowStreak >= VSYS_ALERT_SAMPLES) {
    vsysAlertActive = true;
    logEvent("ALERTA alimentacion baja vsys=" + String(vsysNow_V, 2) +
             "V lte=" + String((int)lteState) +
             " enviosOk=" + String(sesionEnviosOk));
  } else if (vsysAlertActive && vsysNow_V > VSYS_ALERT_CLEAR_V) {
    vsysAlertActive = false;
    logEvent("ALERTA alimentacion restablecida vsys=" + String(vsysNow_V, 2) + "V");
  }
}

void serviceEventLog(uint32_t now) {
  if (elapsed(now, lastEventStatusMs, EVENT_STATUS_MS)) {
    lastEventStatusMs = now;
    logStatusSnapshot();
  }
  if (elapsed(now, lastEventSummaryMs, EVENT_SUMMARY_MS)) {
    lastEventSummaryMs = now;
    logSessionSummary();
  }
}
...
bool loadImuCalibrationFromSd() {
  ...
  // Se distinguen dos cosas que antes se confundian en una sola bandera. El
  // piso se recupera casi siempre; el rumbo solo si el nodo llego a aprender el
  // norte en una sesion anterior. Reportar unicamente el rumbo hacia parecer que
  // la persistencia no funcionaba, cuando en realidad el piso si se restauraba.
  imuCalibrationFloorLoaded = icmAlignment.floorValid || mpuAlignment.floorValid;
...
// I2C / MPU6500
// ============================================================================
// Recuperacion del bus I2C antes de inicializarlo.
//
// Si el RP2040 se reinicia en medio de una lectura, el esclavo puede quedarse
// conduciendo SDA a masa esperando los pulsos de reloj que le faltan. En ese
// estado Wire.begin() no corrige nada y el nodo arranca ciego: cada acceso
// expira en Wire.setTimeout (50 ms) y el lazo se degrada a segundos por vuelta.
// Se observo durante las pruebas del Capitulo 4: la OLED tardaba 1851 ms por
// refresco, equivalente a 37 accesos expirados seguidos.
// La maniobra estandar consiste en emitir hasta nueve pulsos de reloj para que
// el esclavo termine su byte y libere la linea, y cerrar con una condicion de
// paro para dejar el bus en reposo.
void recoverI2cBus() {
  pinMode(I2C_SDA_PIN, INPUT_PULLUP);
  pinMode(I2C_SCL_PIN, INPUT_PULLUP);
  delayMicroseconds(50);

  if (digitalRead(I2C_SDA_PIN) == HIGH) return;  // bus ya libre

  Serial.println("Bus I2C retenido por un esclavo: aplicando recuperacion...");
  for (uint8_t i = 0; i < 9 && digitalRead(I2C_SDA_PIN) == LOW; ++i) {
    pinMode(I2C_SCL_PIN, OUTPUT);
    digitalWrite(I2C_SCL_PIN, LOW);
    delayMicroseconds(5);
    pinMode(I2C_SCL_PIN, INPUT_PULLUP);   // el pull-up lo devuelve a alto
    delayMicroseconds(5);
  }

  // Condicion de paro: SDA pasa de bajo a alto mientras SCL esta alto.
  ...
  Serial.println(digitalRead(I2C_SDA_PIN) == HIGH
                 ? "Bus I2C liberado."
                 : "ADVERTENCIA: el bus I2C sigue retenido.");
}
...
void writeVibrationLog(uint32_t now) {
  ...
  // El siguiente instante se calcula sobre el horario previsto, no sobre el
  // momento real de escritura. Reprogramar desde 'now' acumulaba el tiempo que
  // consume la propia vuelta del lazo (unos 7 ms entre la escritura en microSD
  // y el refresco de la OLED), y el registro se estabilizaba en 17.6 Hz en vez
  // de 20 Hz. Si el nodo se atrasa mas de tres periodos se resincroniza, para
  // no dispararse una rafaga de recuperacion tras un bloqueo largo.
  lastVibLogMs += VIB_LOG_INTERVAL_MS;
  if ((uint32_t)(now - lastVibLogMs) > 3 * VIB_LOG_INTERVAL_MS) lastVibLogMs = now;
  ...
  vibFile.print(mpuMag_g, 4); vibFile.print(',');

  // Extremos de la alimentacion durante este intervalo de 50 ms. Guardar el
  // minimo y el maximo, y no una lectura instantanea, es lo que permite ver
  // una caida de pocos milisegundos coincidente con una rafaga del modem.
  vibFile.print(vsysMin_V, 3); vibFile.print(',');
  vibFile.println(vsysMax_V, 3);
  vsysMin_V = 99.0f;
  vsysMax_V = 0.0f;
...
bool initializeSdStorage(bool startup) {
  ...
  // Varios intentos con pausa entre ellos.
  //
  // Antes se intentaba montar la tarjeta una sola vez, de inmediato. Alimentado
  // por USB eso funciona siempre, pero desde el modulo de baterias el riel sube
  // mas despacio y con mas rizado, y la tarjeta puede no estar lista todavia.
  // Si ese unico intento fallaba, sdOK quedaba en falso y TODO el registro se
  // apagaba en silencio: ni bitacora ni archivo de vibracion, con el nodo
  // operando con normalidad durante horas. Ocurrio en las sesiones con bateria
  // del 29 y 30 de agosto de 2026, que no dejaron un solo byte en la tarjeta.
  bool montada = false;
  for (uint8_t intento = 1; intento <= SD_MOUNT_ATTEMPTS && !montada; ++intento) {
    montada = SD.begin(SD_CS_PIN);
    if (!montada) {
      Serial.print("SD.begin fallo, intento ");
      Serial.print(intento);
      Serial.print(" de ");
      Serial.println(SD_MOUNT_ATTEMPTS);
      SD.end(false);
      delay(SD_MOUNT_RETRY_MS);
    }
  }

  if (!montada) {
    setSdUnavailable("SD.begin fallo tras varios intentos");
    return false;
  }
...
void updateOled() {
  ...
  // VSYS al lado del estado de la SD, y no por adorno.
  //
  // Alimentado por baterias no hay consola USB: la OLED es el unico
  // instrumento. Un "SD:F" fijo acompanado de un riel bajo apunta al margen de
  // tension del modulo de la tarjeta, no a la tarjeta ni al bus. Con USB el
  // riel se midio en 4.74 V; una lectura notablemente menor con baterias es el
  // dato que separa una causa de la otra.
  display.print(' ');
  display.print(vsysNow_V, 1);
...
void enterLteState(LteState state) {
  if (state == LTE_FAIL) lteFailedFromState = lteState;
  lteState = state;
...
// Pulso sobre PWRKEY con la temporizacion de la version anterior. Bloquea, de
// modo que solo debe usarse durante setup().
void modemPulsoPwrkey() {
...
// Pregunta al modem si ya esta despierto. Devuelve true en cuanto recibe "OK".
// No pasa por parseModemLine a proposito: durante el arranque el SIM7600 emite
// "RDY" y "+CPIN: READY" de forma legitima, y contarlos como reinicios
// espontaneos ensuciaria la bitacora.
bool modemRespondeAT(uint32_t esperaMs) {
...
// Sondea con AT hasta que el modem conteste o se agote la ventana. Se usa tras
// el pulso de PWRKEY, cuando el SIM7600 esta arrancando y todavia no atiende.
bool modemEsperaAT(uint32_t ventanaMs) {
  uint32_t inicio = millis();
  while (millis() - inicio < ventanaMs) {
    if (modemRespondeAT(2000)) {
      Serial.print("SIM7600 responde tras ");
...
void parseModemLine(const String &line) {
  ...
  // Deteccion de reinicios del modem.
  //
  // "RDY" y "+CPIN: READY" son mensajes espontaneos que el SIM7600 emite
  // cuando termina de arrancar. Recibirlos una vez tras el encendido es
  // normal; recibirlos de nuevo mas adelante significa que el modem se
  // reinicio solo, y la causa habitual es que su rama de alimentacion no
  // sostiene el pico de corriente del registro en red.
  // El Pico no puede medir ese riel, asi que anotar el evento es la unica
  // forma de dejar constancia cuando el nodo opera sin consola conectada.
  if (millis() > MODEM_RESET_GRACE_MS &&
      (line == "RDY" || line.indexOf("+CPIN: READY") >= 0)) {
    if (!elapsed(millis(), lastModemResetMs, 5000)) return;  // una vez por evento
    lastModemResetMs = millis();
    sesionReiniciosModem++;
    logEvent("MODEM REINICIO detectado (" + line + ") total=" +
             String(sesionReiniciosModem) +
             " lte=" + String((int)lteState) +
             " vsysPico=" + String(vsysNow_V, 2) + "V");
  }
...
String buildPostData() {
  ...
  String status = "SRC_" + String((int)txSource) +
                  ...
                  // String(float, 0) de Arduino formatea con dtostrf y un ancho
                  // minimo de dos caracteres, asi que un rumbo de un solo digito
                  // sale con un espacio delante: "_HDG_ 4" en vez de "_HDG_4".
                  // La plataforma no reconoce ese campo y descarta la fuente y el
                  // estado de la IMU de esa entrada: ocurrio en las entradas 127,
                  // 130 y 148 del 20 de septiembre de 2026, con rumbos de 3, 9 y
                  // 4 grados. Redondear a entero elimina el relleno.
                  "_HDG_" + String((int)lroundf(txHeading_deg)) +
...
void lteStep() {
  ...
  // ==========================================================================
  // VARIANTE DE PRUEBA - NO USAR EN OPERACION
  // ==========================================================================
  // Apaga el modem de verdad con AT+CPOF a los 90 s. A partir de ahi el SIM7600
  // esta APAGADO, no solo sordo, y con el umbral bajado a 3 la reanimacion se
  // dispara hacia el segundo 105, antes de que la tarjeta lo vuelva a encender
  // sola, cosa que en la corrida anterior tardo unos 30 s.
  //
  // Es el caso que la correccion arregla. Con el modem apagado, un unico pulso
  // de PWRKEY debe encenderlo y la sonda debe responder en cuanto termine de
  // arrancar: la linea final debe decir "reanimado con 1 pulso". Con la
  // secuencia anterior, de dos pulsos encadenados sin preguntar en medio, el
  // primero lo habria encendido y el segundo lo habria vuelto a apagar.
  static bool cpofEnviado = false;
  if (!cpofEnviado && millis() > 90000UL && lteState == LTE_IDLE) {
    cpofEnviado = true;
    logEvent("PRUEBA apagando el modem con AT+CPOF");
    modemSend("AT+CPOF");
  }
  ...
  if (lteState == LTE_IDLE) {
    ...
    bool timeForNew = !havePending && canBuildNewPayload &&
                      ...
                      elapsed(now, lastAcceptedSendMs, TS_MIN_GAP_MS);
    // El piso tambien se aplica a los reintentos: un ciclo que falla rapido
    // podria encadenar un reintento dentro de la ventana de 15 s del ultimo
    // envio aceptado, y esa entrega se perderia sin dejar rastro.
    bool timeForRetry = havePending &&
                        elapsed(now, lastRetryMs, RETRY_AFTER_MS) &&
                        elapsed(now, lastAcceptedSendMs, TS_MIN_GAP_MS);
  ...
  switch (lteState) {
    ...
    case LTE_DONE:
      sesionEnviosOk++;
      logEvent("HTTP OK codigo=" + String(httpCode) +
               " ciclo=" + String(millis() - lteCycleStartMs) + "ms" +
               " cuerpo=" + String(httpResponseLength) + "B" +
               " envio=" + String(sesionEnviosOk));
      // ThingSpeak no senala con un error HTTP que haya descartado una
      // actualizacion por llegar antes de los 15 s: responde 200 con el cuerpo
      // "0", un unico byte, y no crea la entrada. Una entrada aceptada devuelve
      // su entry_id, que son dos o mas bytes. Sin esta comprobacion el nodo
      // daria por entregado un paquete que nunca llego a almacenarse.
      if (httpResponseLength <= 1) {
        logEvent("AVISO ThingSpeak descarto la actualizacion (cuerpo=" +
                 String(httpResponseLength) + "B): llego antes de 15 s");
      }
      clearPendingAfterSuccess();
      pulseTsLed();
      lastSendMs = millis();
      lastAcceptedSendMs = lastSendMs;
      modemDeafStreak = 0;
      enterLteState(LTE_IDLE);
      break;

    case LTE_FAIL:
      sesionEnviosFallidos++;
      // Se anota el estado en que se corto el ciclo: es lo que distingue una
      // falla de red de una de servidor cuando no hay consola conectada.
      logEvent("HTTP FALLO codigo=" + String(httpCode) +
               " estadoPrevio=" + String((int)lteFailedFromState) +
               " ciclo=" + String(millis() - lteCycleStartMs) + "ms" +
               " fallidos=" + String(sesionEnviosFallidos));
      ...
      // Cuando el ciclo se corta una y otra vez en LTE_WAIT_AT, el SIM7600 no
      // esta contestando ni a un AT pelon. Mandarle AT+CFUN o AT+CRESET no
      // sirve: viajan por el mismo puerto serie que dejo de responder. La unica
      // palanca que queda es PWRKEY, que es una linea fisica y ya esta cableada
      // en GP6. Como PWRKEY conmuta en vez de encender, hacen falta dos pulsos:
      // el primero apaga el modem, que seguia alimentado, y el segundo lo
      // vuelve a encender.
      if (lteFailedFromState == LTE_WAIT_AT) {
        if (modemDeafStreak < 255) modemDeafStreak++;
      } else {
        modemDeafStreak = 0;
      }
      if (modemDeafStreak >= MODEM_DEAF_STREAK_TRIGGER &&
          (lastModemRecoveryMs == 0 ||
           elapsed(millis(), lastModemRecoveryMs, MODEM_RECOVERY_COOLDOWN_MS))) {
        sesionRecuperacionesModem++;
        modemRecoveryStartMs = millis();
        lastModemRecoveryMs = modemRecoveryStartMs;
        logEvent("MODEM mudo tras " + String((int)modemDeafStreak) +
                 " esperas agotadas; reanimacion por PWRKEY numero " +
                 String(sesionRecuperacionesModem));
        modemRecoveryPulses = 0;
        enterLteState(LTE_RECOVER_PULSE);
        break;
      }
      enterLteState(LTE_IDLE);
      break;
    ...
    // ======================================================================
    // REANIMACION DEL MODEM
    // ======================================================================
    // Toda la secuencia corre dentro de la maquina de estados, sin bloquear.
    // modemPulsoPwrkey() no sirve aqui: detiene el lazo 5.5 s, y a 20 Hz eso
    // son unas 110 muestras perdidas, ademas del tipo de inanicion del lazo que
    // causo el defecto M4 en agosto.
    // PWRKEY conmuta: el mismo pulso que enciende un modem apagado apaga uno
    // encendido. Por eso la secuencia es pulsar, preguntar, y solo si sigue
    // mudo volver a pulsar; no dos pulsos seguidos.
    //
    // Dos pulsos seguidos solo funcionan si el modem estaba encendido pero
    // sordo: el primero lo apaga y el segundo lo enciende. Si hubiera estado
    // apagado de verdad -por ejemplo tras un corte en su propia rama- el
    // primero lo encenderia y el segundo lo volveria a apagar, dejandolo peor
    // que al empezar. Preguntando entre pulso y pulso, la secuencia termina con
    // el modem encendido venga del estado que venga.
    case LTE_RECOVER_PULSE:
      digitalWrite(PWRKEY, HIGH);
      if (lteTimedOut(PWRKEY_PULSE_MS)) {
        digitalWrite(PWRKEY, LOW);
        modemRecoveryPulses++;
        enterLteState(LTE_RECOVER_SETTLE);
      }
      break;

    case LTE_RECOVER_SETTLE:
      // Margen corto y comun a los dos casos. Si el pulso encendio el modem,
      // las primeras sondas fallaran hasta que termine de arrancar, que es lo
      // que cubre la ventana de MODEM_ESPERA_AT_MS.
      if (lteTimedOut(PWRKEY_SETTLE_OFF_MS)) {
        modemRecoveryStartMs = millis();   // la ventana cuenta desde aqui
        enterLteState(LTE_RECOVER_PROBE);
      }
      break;

    case LTE_RECOVER_PROBE:
      modemSend("AT");
      enterLteState(LTE_RECOVER_WAIT_PROBE);
      break;

    case LTE_RECOVER_WAIT_PROBE:
      // El RDY que el modem emite al arrancar queda registrado por
      // parseModemLine como "MODEM REINICIO detectado". En esta ventana es
      // esperado: lo provocamos nosotros, y aparece junto a la linea de
      // reanimacion que lo precede.
      if (rxOK) {
        logEvent("MODEM reanimado con " + String((int)modemRecoveryPulses) +
                 " pulso(s) de PWRKEY");
        modemDeafStreak = 0;
        modemRecoveryPulses = 0;
        enterLteState(LTE_IDLE);
      } else if (lteTimedOut(MODEM_RECOVERY_PROBE_MS)) {
        if (!elapsed(millis(), modemRecoveryStartMs, MODEM_ESPERA_AT_MS)) {
          enterLteState(LTE_RECOVER_PROBE);        // sigue dentro de la ventana
        } else if (modemRecoveryPulses < MODEM_RECOVERY_MAX_PULSES) {
          logEvent("MODEM mudo tras el pulso " + String((int)modemRecoveryPulses) +
                   "; se aplica otro");
          enterLteState(LTE_RECOVER_PULSE);
        } else {
          // Se deja como esta y la FSM seguira reintentando el ciclo normal.
          // El enfriamiento impide encadenar reanimaciones sin tregua.
          logEvent("MODEM sigue mudo tras " + String((int)modemRecoveryPulses) +
                   " pulsos; se reintentara mas tarde");
          modemDeafStreak = 0;
          modemRecoveryPulses = 0;
          enterLteState(LTE_IDLE);
        }
...
void setup() {
  ...
  analogReadResolution(12);   // VSYS se lee con los 12 bits del ADC
  for (uint8_t i = 0; i < 8; ++i) sampleVsys();   // asienta el ADC

  Serial.println();
  Serial.print("Iniciando ");
  Serial.println(FW_VERSION);
  Serial.print("Causa del ultimo reinicio: "); Serial.println(resetReasonName());

  recoverI2cBus();   // libera el bus si un reinicio dejo a un esclavo colgado
  ...
  // Marca temprana, en cuanto la tarjeta esta disponible y antes de las
  // calibraciones, que tardan unos once segundos. Si el nodo se reinicia
  // durante ese tramo, el bloque de arranque completo nunca llegaria a
  // escribirse y la sesion quedaria sin rastro.
  logEvent("----------------------------------------");
  logEvent(String("INICIO ") + FW_VERSION +
           " causa=" + resetReasonName() +
           " sd=" + (sdOK ? "si" : "no"));
  ...
  // ==========================================================================
  // BLOQUE DE ARRANQUE EN LA BITACORA
  // ==========================================================================
  // Se escribe ANTES de encender el SIM7600, y esa posicion es deliberada.
  //
  // El pulso de encendido del modem provoca una demanda de corriente cercana a
  // 2 A. Si la alimentacion no la sostiene, el RP2040 se reinicia durante esa
  // ventana y vuelve a empezar, indefinidamente. Con el bloque escrito despues
  // del modem, ese bucle no dejaba ni una sola linea en la tarjeta: el sintoma
  // era una microSD completamente vacia tras horas de operacion, sin ninguna
  // pista de que habia pasado. Ocurrio en la sesion del 28 de agosto de 2026.
  //
  // Escribiendolo aqui, cada intento de arranque queda registrado con su causa
  // de reinicio. Una tarjeta con decenas de bloques ARRANQUE seguidos de
  // BROWNOUT es el diagnostico directo de una alimentacion insuficiente.
  logEvent("========================================");
  logEvent(String("ARRANQUE ") + FW_VERSION +
           " tren=" + String(ID_TREN) + " nodo=" + String(ID_NODO));
  logEvent(String("REINICIO causa=") + resetReasonName());
  // Se remuestrea aqui mismo: el valor de las muestras de calentamiento del
  // ADC corresponde a la rampa de encendido y no al estado real del riel.
  sampleVsys();
  logEvent("ALIMENTACION vsys=" + String(vsysNow_V, 2) + "V");
  logEvent(String("SD montada=") + (sdOK ? "si" : "no") +
           " calibracionPiso=" + String(imuCalibrationFloorLoaded ? "si" : "no") +
           " calibracionRumbo=" + String(imuCalibrationLoaded ? "si" : "no"));
  logEvent(String("SENSORES mpu=") + (mpuPresent ? "si" : "no") +
           " icm=" + (icmPresent ? "si" : "no") +
           " gnss=" + (gpsPresent ? "si" : "no") +
           " oled=" + (oledOK ? "si" : "no"));
  logEvent("CALIBRACION gravedadRefIcm=" + String(icmGravityRef_g, 4) +
           " gravedadRefMpu=" + String(mpuGravityRef_g, 4) +
           " pisoIcm=" + String(icmAlignment.floorValid ? 1 : 0) +
           " pisoMpu=" + String(mpuAlignment.floorValid ? 1 : 0) +
           " norte=" + String(northReferenceValid ? 1 : 0));
  logEvent("GIRO biasX=" + String(icmGyroBiasSensorX_dps, 4) +
           " biasY=" + String(icmGyroBiasSensorY_dps, 4) +
           " biasZ=" + String(icmGyroBiasSensorZ_dps, 4));

  // Serial1 usa GP0 (TX) y GP1 (RX) en la configuracion del proyecto.
  //
  // La FIFO de recepcion por omision es de 32 bytes, que a 115200 baudios se
  // llena en 2.8 ms. El lazo principal se bloquea rutinariamente mas que eso
  // (refresco de la OLED ~29 ms, escritura en microSD 9 a 11 ms), de modo que
  // las respuestas del modem se perdian a medio camino: durante las pruebas se
  // observo "+IP ERROR: Network is already" en vez de "...already opened",
  // y la FSM caia en LTE_FAIL por agotamiento de espera aunque la red estuviera
  // disponible. setFIFOSize debe llamarse antes de begin para que surta efecto.
  Serial1.setFIFOSize(1024);
  modem->begin(115200);

  // Encendido del SIM7600, condicionado a que no este ya encendido.
  //
  // PWRKEY no es una senal de "encender": es un conmutador. El mismo pulso que
  // enciende un modem apagado apaga uno que ya estaba encendido. El RP2040 se
  // reinicia por software al recargar el firmware, por el perro guardian o al
  // quitar el USB, y en todos esos casos el SIM7600 conserva su propia
  // alimentacion y sigue encendido: pulsar sin preguntar lo apagaba. Como
  // ninguna otra ruta vuelve a pulsar PWRKEY, el nodo quedaba sin radio hasta
  // el siguiente corte real de energia, contestando cada ciclo con
  // "HTTP FALLO codigo=-1 estadoPrevio=2", que es la espera agotada aguardando
  // la respuesta a AT. Se observo el 6 y el 13 de septiembre de 2026.
  pinMode(PWRKEY, OUTPUT);
  digitalWrite(PWRKEY, LOW);

  if (modemRespondeAT(1500)) {
    Serial.println("SIM7600 ya encendido; se omite el pulso de PWRKEY.");
    logEvent("MODEM ya encendido, pulso omitido");
  } else {
    // Un solo pulso, nunca dos. Un segundo pulso sobre un modem que ya se
    // encendio pero todavia no termina de arrancar lo apaga de nuevo, que es
    // justo el fallo que esta rutina existe para evitar. Si tras la ventana
    // sigue mudo se deja como esta y la FSM seguira reintentando: es preferible
    // un ciclo de transmision fallido a apagar la radio.
    Serial.println("SIM7600 sin respuesta; pulso unico de PWRKEY.");
    modemPulsoPwrkey();
    bool despierto = modemEsperaAT(MODEM_ESPERA_AT_MS);
    logEvent(despierto ? "MODEM encendido por pulso de PWRKEY"
                       : "MODEM mudo tras el pulso; la FSM seguira reintentando");
  }
  Serial.println("SIM7600 inicializado; FSM en espera.");
  // Si esta linea falta y la anterior existe, el nodo no sobrevivio al
  // encendido del modem: la alimentacion no sostuvo el pico de corriente.
  sampleVsys();
  logEvent("MODEM encendido, vsys=" + String(vsysNow_V, 2) + "V");
